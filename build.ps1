param(
    [string]$BeatSaberDir = "D:\beat saber\BSManager\BSInstances\1.44.1"
)

$ErrorActionPreference = "Stop"

Write-Host "ToyanBomb v1.1.6 for Beat Saber 1.44.1 Player View Message build"
...
dotnet build ".\ToyanBomb\ToyanBomb.csproj" `
    -c Release `
...
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "Build failed. Copy the error output and send it back." -ForegroundColor Red
    exit $LASTEXITCODE
}

$dll = ".\ToyanBomb\bin\Release\net472\ToyanBomb.dll"
...
Write-Host "  IMPORTANT: delete any loose $BeatSaberDir\Plugins\manifest.json from the previous test."